...
// {
//   printf("Initializing a fucking kalman filter struct \n");
...
//  */
// /*
// extern void linear_kalman_filter_predict_and_update(struct linear_kalman_filter *filter, float *U, float *Y)
// {
...
//   float_vect_diff(err, Y, err, filter->m); // err = Y - C * X
//   float_vact_diff(err, err, err2, filter->m) // err = Y - C * X - D * U
//   float_mat_vect_mul(dx_err, _K, err, filter->n, filter->m); // K * err
//   float_vect_sum(filter->X, filter->X, dx_err, filter->n); // X + dx_err
...
//  */
// /*
// extern void linear_kalman_filter_predict_and_update_MATLAB(struct linear_kalman_filter *filter, float *U, float *Y)
// {
//   // prediction step
//   float AX[filter->n];
//   /*
...
//   flaot_mat_mul_copy(_tmp1, _A, _tmp1, filter->n, filter->m, filter->m) // tmp1 = A * P * C
...
//   float_mat_sum(_Z, _Z, _I) // (I-M*C)*P*(I-M*C)' + M*R*M' 
